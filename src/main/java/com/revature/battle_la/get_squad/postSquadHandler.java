...
public class postSquadHandler implements RequestHandler<APIGatewayProxyRequestEvent, APIGatewayProxyResponseEvent> {
    ...
    public APIGatewayProxyResponseEvent handleRequest(APIGatewayProxyRequestEvent reqEvent, Context context) {
        ...
        Bro bro = mapper.fromJson(reqEvent.getBody(), Bro.class);
        boolean yay = squadRepo.createBro(bro);
        boolean nay = !yay;

        APIGatewayProxyResponseEvent respEvent = new APIGatewayProxyResponseEvent();

        respEvent.setBody(mapper.toJson(bro));

        if(yay) respEvent.setStatusCode(201);
        if(nay) respEvent.setStatusCode(400);
